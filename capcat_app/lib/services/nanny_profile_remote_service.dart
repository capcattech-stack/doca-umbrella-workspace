...
class NannyProfileRemoteService {
  static Future<ServiceResponse<NannyProfile>> startNanny() async {
    ...
    try {
      final response = await ApiService.startNanny(token);
      ...
      if (!response.isSuccess) {
        return ServiceResponse(
          isSuccess: false,
          message: serverMessage ?? 'Không tải được thông tin bảo mẫu',
        );
      }
      ...
      if (data is! Map<String, dynamic>) {
        return ServiceResponse(
          isSuccess: false,
          message: 'Dữ liệu bảo mẫu không hợp lệ',
        );
      }

      return ServiceResponse(
        isSuccess: true,
        data: NannyProfile.fromJson(data),
      );
    } catch (_) {
      return ServiceResponse(
        isSuccess: false,
