...
### 2.2. Quy trình Xử lý Lỗi Ảnh & Tự động tạo Ảnh Minh Họa Ghibli (Error & Skip Fallback Flow)
...
[ Sen Chọn/Chụp Ảnh Boss ]
   ...
   (ML Kit Quét Offline)
           ...
           +---> [KHÔNG CÓ nhãn Dog/Cat] ---> Hiện Hộp thoại cảnh báo dễ thương
             ...
             +---------------------------------------+---------------------------------------+
             |                                                                               |
    [Nút 1: Chọn ảnh khác]                                                          [Nút 2: Bỏ qua và dùng Ghibli Art]
             |                                                                               |
    Mở lại Gallery/Camera                                                Tự động gán Ảnh màu nước Ghibli cục bộ (0đ)
...
*   **Nút 2 - Bỏ qua & Dùng ảnh Ghibli Art:** Chuyển tiếp Sen trực tiếp sang bước điền mô tả, app sẽ tự động gán một bức tranh minh họa màu nước phong cách Ghibli cực kỳ nghệ thuật.

#### B. Cơ chế Tự động Tạo Ảnh Minh Họa Ghibli 0đ (Zero-Cost Ghibli Watercolor Auto-Generation)
Để duy trì **chi phí 0đ vĩnh viễn** và **độ trễ 0ms** (không gọi DALL-E hay Midjourney server), Capcat chuẩn bị sẵn **Kho Tranh Minh Họa Màu Nước Phong Cách Ghibli Cục Bộ (Local Ghibli Watercolor Presets)** được phác họa mộc mạc và tô màu ấm áp bởi đội ngũ họa sĩ.

Hệ thống sẽ **tự động ghép nối (Render) ảnh đại diện hoàn hảo** dựa trên thông số Sen nhập:

$$\text{Avatar Path} = \text{assets/illustrations/ghibli/} + \text{species} + \text{"\_"} + \text{breed} + \text{"\_"} + \text{coat\_color} + \text{".png"}$$

*   *Ví dụ 1:* Sen chọn Loài = `Cat`, Giống = `British Shorthair` (ALN), Màu lông = `Silver Tabby` (Xám mướp) $\rightarrow$ App tự động gán bức tranh Ghibli cực xinh: `assets/illustrations/ghibli/cat_british_shorthair_silver_tabby.png`.
*   *Ví dụ 2:* Sen chọn Loài = `Dog`, Giống = `Corgi`, Màu lông = `Cream Gold` (Vàng kem) $\rightarrow$ App tự động gán: `assets/illustrations/ghibli/dog_corgi_cream_gold.png`.
*   *Trường hợp không khớp giống đặc thù:* Hệ thống tự động fallback về tranh minh họa Ghibli đáng yêu chung của loài đó (Ví dụ: `assets/illustrations/ghibli/cat_default_ginger_orange.png`).
