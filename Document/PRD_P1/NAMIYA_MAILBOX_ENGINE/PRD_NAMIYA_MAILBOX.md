...
| (Pets nhắc hẹn tự nhiên) |       | (Trình soạn thảo Notion)  |       |  (Hòm thư gỗ MUJI)  |
+-----------------------+       +---------------------------+       +---------------------------+
```
...
### 3. Đồng bộ hóa Lời nhắc Cozy Chat và Trạng thái Chặn (Rate Limit & Chat Integration)
*   **Vấn đề:** Tránh trường hợp Boss AI trong Cozy Chat phát hiện tâm trạng Sen buồn và gợi ý viết thư Namiya, nhưng khi Sen mở Namiya Mailbox thì lại bị chặn do đã gửi 1 thư trong vòng 24 giờ trước đó (Rate Limit).
*   **Giải pháp:** 
    *   Trước khi gửi tin nhắn gợi ý đi Namiya, Cozy Chat Engine sẽ kiểm tra flag rate limit cục bộ (`is_namiya_rate_limited` - được lưu trong Local Database/Preferences khi gửi thư thành công).
    *   Nếu người dùng đang bị giới hạn, Boss AI sẽ chuyển lời thoại gợi ý sang một hành động khác phù hợp hơn (ví dụ: rủ Sen mở Thùng Sữa đọc lại những thư hồi âm ấm áp cũ, hoặc bật nhạc thư giãn trên Kệ Muji).
